...
//                    ,templet:"#nameDemo"
                ...
                ,{field: 'authenticationStatus' ,width:"10%",align:'center', title: '审核状态',templet:"#authenticationStatusTpl"}
...
//                }
        ...
        var active = {
            batchPass: function(){
                ...
                layer.confirm("确定通过吗?",function(resdata){
                    $.post('<%=path%>/user/batchAuthentication',{ids:ids,authenticationStatus:2},
                        function (res) {
                            if (res.code==0){
                                refreshTable();
                                layer.msg("成功");
            ...
            ,unBatchPass: function(){
                ...
                layer.confirm("确定不通过吗?",function(resdata){
                    $.post('<%=path%>/user/batchAuthentication',{ids:ids,authenticationStatus:3},
                        function (res) {
                            if (res.code==0){
                                refreshTable();
                                layer.msg("成功");
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'pass') {
                layer.confirm("确定通过吗?",function(resdata){
                    $.post('<%=path%>/user/authentication',{id:data.id,authenticationStatus:2},
                        function (res) {
                            if (res.code==0){
                                refreshTable();
                                layer.msg("成功");
            ...
            if (obj.event === 'unPass') {
                layer.confirm("确定不通过吗?",function(resdata){
                    $.post('<%=path%>/user/authentication',{id:data.id,authenticationStatus:3},
                        function (res) {
                            if (res.code==0){
                                refreshTable();
                                layer.msg("成功");
