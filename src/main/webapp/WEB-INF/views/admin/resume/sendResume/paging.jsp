    ...
    <div class="layui-card">
        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="table-useradmin-webuser">
                <a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail"><i class="layui-icon layui-icon-search"></i>查看简历</a>
                <a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detailJob"><i class="layui-icon layui-icon-search"></i>查看职位</a>
...
<script>
    ...
    layui.use(['form','layer','table'], function(){
        ...
        table.render({
            elem: '#demo'
            // ,height: 700
            ,url: '<%=path%>/sendResume/paging' //数据接口
            ,method:"post"
            ,where: {status: 1}
            ,page: true //开启分页
            ,limit:10
...
////                    ,templet:"#nameDemo"
//                }
                ,{field: 'name' ,width:"10%",align:'center', title: '用户名称'}
                ,{field: 'phone' ,width:"10%",align:'center', title: '用户电话'}
                ,{field: 'jobName' ,width:"10%",align:'center', title: '职位名称'}
                ,{field: 'enterpriseName' ,width:"10%",align:'center', title: '企业名称'}
//                ,{field: 'userResumeId' ,width:"10%",align:'center', title: '用户简历ID'}
        ...
        table.on('tool(demo)', function(obj){
            ...
            if (obj.event === 'detail') {
                popUpBox = layer.open({
                    ...
                    maxmin: true,
                    title: '简历详情',
                    content:'<%=path%>/resume/userResume/jumpDetail?id='+data.userResumeId
                })
            }
            // 查看详情
            if (obj.event === 'detailJob') {
                popUpBox = layer.open({
                    type: 2,
                    area: ['100%', '100%'],
                    fixed:false,
                    maxmin: true,
                    title: '职位详情',
                    content:'<%=path%>/invite/issueJob/jumpDetail?id='+data.issueJobId
                })
            }
            // 通过
            if (obj.event === 'update') {
